...
let getFinalAgentPrompt = (agentRunState, commands, objectiveForCurrentAction) => {
    ...
    if (is_using_kcpp_with_fs()) {
        ...
        let embeddedFunctionGuidance = [
            `Embedded content can call eight helper functions. Use the right one for the task:`,
            ...
            "```",
            `2) generateTextFromAI(prompt, keepThinkingTags?)`,
            ...
            `- Inputs: prompt is required text. keepThinkingTags is optional boolean (default false); set to true to preserve any internal <thinking>...</thinking> tags in the returned text.`,
            `- Behavior: Returns a text string (or empty string on failure). Thinking tags are stripped by default.`,
            `- Example:`,
            "```js",
            ...
            `// With thinking tags preserved:`,
            `let raw = await (window?.opener || window?.parent).generateTextFromAI("Reason step by step about this problem.", true)`,
            "```",
            ...
            "```",
            `4) prepareMusicFromAI(caption)`,
            `- Purpose: Prepare music generation fields from a short caption using the Kobold music prepare endpoint.`,
            `- Use when: You want suggested lyrics/settings before generating music.`,
            `- Inputs: caption is required text describing the song idea.`,
            `- Behavior: Returns JSON with prepared music fields (caption/lyrics/BPM/key/etc).`,
            `- Example:`,
            "```js",
            `let prep = await (window?.opener || window?.parent).prepareMusicFromAI("melancholic synthwave night drive")`,
            `console.log(prep)`,
            "```",
            `5) generateMusicFromAI(options?)`,
            `- Purpose: Generate music audio from settings and return a playable data URL.`,
            `- Use when: Embedded content needs generated music without writing to filesystem.`,
            `- Inputs: options object may include caption, lyrics, bpm, duration, keyscale, timesignature, vocal_language, inference_steps, and optional reference_audio_data (base64 or data URL).`,
            `- Behavior: Returns a string like "data:audio/wav;base64,..." suitable for <audio src="...">.`,
            `- Example:`,
            "```js",
            `let audioSrc = await (window?.opener || window?.parent).generateMusicFromAI({ caption: "uplifting orchestral finale", duration: 45 })`,
            `if (audioSrc) document.querySelector("#musicPreview").src = audioSrc`,
            "```",
            `6) generateTTSFromAI(textToSay, voice?)`,
            `- Purpose: Generate speech audio from text and return a playable data URL.`,
            `- Use when: Embedded content needs TTS output for in-page playback or download.`,
            `- Inputs: textToSay is required text. voice is optional and should be one of the available Kobold voices.`,
            `- Behavior: Returns a string like "data:audio/wav;base64,..." suitable for <audio src="...">.`,
            `- Example:`,
            "```js",
            `let ttsSrc = await (window?.opener || window?.parent).generateTTSFromAI("Welcome aboard.", "kobo")`,
            `if (ttsSrc) document.querySelector("#voicePreview").src = ttsSrc`,
            "```",
            `7) getAvailableVoicesFromAI()`,
            `- Purpose: Fetch the currently available Kobold voice names.`,
            `- Use when: UI needs to populate a voice selector before calling generateTTSFromAI.`,
            `- Inputs: none.`,
            `- Behavior: Returns a JSON array of voice strings.`,
            `- Example:`,
            "```js",
            `let voices = await (window?.opener || window?.parent).getAvailableVoicesFromAI()`,
            `console.log(voices)`,
            "```",
            `8) generateObjectFromAI(prompt, objectStructure?)`,
            ...
            `- Prefer prepareMusicFromAI + generateMusicFromAI for music workflows where settings need to be reviewed or edited.`,
            `- Prefer generateTTSFromAI for TTS audio you want to play in embedded content.`,
            `- Use getAvailableVoicesFromAI to populate selectable voice lists before TTS generation.`,
